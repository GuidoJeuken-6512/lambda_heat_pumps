...
- **`cycle_energy_persist.json` could silently regress an energy sensor's cumulative value back down after a restart** — most visibly as `Energy sensor <entity>: Persist last_hour_value (X) > energy_value (0.00), correct to energy_value` in the log for `_hourly` sensors, but for `_total` sensors (and the `_total`-period COP sensors that read from them) the same regression happened **without any warning at all**, since `ENERGY_PERIOD_CONFIG` doesn't cover the `total` period. `_apply_persisted_energy_state()` (`sensor.py`) unconditionally trusted whatever `energy_value` was in the coordinator's own `cycle_energy_persist.json` snapshot, overwriting the value `restore_state()` had just correctly restored from Home Assistant's own Recorder-based history. That snapshot is only refreshed when *some* entity's dirty flag triggers a persist write, so an entity that spent time failing to be added to Home Assistant (e.g. every sub-device sensor during the `via_device` outage above, in any session before that fix) never ran `set_energy_value()` and kept the JSON frozen at its constructor default of `0.0` — regressing the real value back to `0.0` on the next successful restart. Confirmed live on a running installation via the Home Assistant MCP server: a `_total` sensor correctly restored to `2518.51` kWh from Recorder history was silently reset to `0.0` by this exact path, cascading into the corresponding `_total`-period COP sensor also dropping to `0.0`. `_apply_persisted_energy_state()` now discards the entire persisted snapshot (not just `energy_value`) whenever it is lower than the value already restored — `energy_value` is a monotonically increasing cumulative counter everywhere in this integration (see `set_energy_value()`'s existing "never decrease" guard), and that invariant now holds during restore too, for every period.
...
- **`cycle_energy_persist.json` konnte den kumulativen Wert eines Energie-Sensors nach einem Neustart still zurückwerfen** — am sichtbarsten als `Energy sensor <entity>: Persist last_hour_value (X) > energy_value (0.00), correct to energy_value` im Log für `_hourly`-Sensoren, aber bei `_total`-Sensoren (und den davon abhängigen `_total`-COP-Sensoren) passierte derselbe Rückfall **ganz ohne jede Warnung**, da `ENERGY_PERIOD_CONFIG` die Periode `total` nicht kennt. `_apply_persisted_energy_state()` (`sensor.py`) vertraute bedingungslos dem `energy_value` aus dem eigenen `cycle_energy_persist.json`-Snapshot des Coordinators und überschrieb damit den Wert, den `restore_state()` gerade erst korrekt aus Home Assistants eigener Recorder-Historie wiederhergestellt hatte. Dieser Snapshot wird nur aktualisiert, wenn der Dirty-Flag *irgendeiner* Entity einen Persist-Write auslöst — bei einer Entity, die zeitweise nicht erfolgreich zu Home Assistant hinzugefügt wurde (z. B. jeder Sub-Device-Sensor während des oben beschriebenen `via_device`-Ausfalls, in jeder Session vor diesem Fix), blieb er deshalb beim Konstruktor-Default `0.0` eingefroren — `set_energy_value()` lief für sie nie. Beim nächsten erfolgreichen Neustart wurde der echte Wert dadurch auf `0.0` zurückgeworfen. Live an einer laufenden Installation über den Home-Assistant-MCP-Server bestätigt: ein `_total`-Sensor, korrekt aus der Recorder-Historie auf `2518.51` kWh restauriert, wurde über genau diesen Pfad still auf `0.0` zurückgesetzt, was den zugehörigen `_total`-COP-Sensor ebenfalls auf `0.0` mitriss. `_apply_persisted_energy_state()` verwirft jetzt den kompletten persistierten Snapshot (nicht nur `energy_value`), sobald dieser niedriger ist als der bereits restaurierte Wert — `energy_value` ist überall in dieser Integration ein monoton steigender kumulativer Zähler (siehe die bestehende „nie verringern"-Absicherung in `set_energy_value()`), und diese Invariante gilt jetzt auch beim Restore, für jede Periode.
